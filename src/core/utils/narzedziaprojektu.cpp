...
// w utils/dxfinwentaryzacja.cpp - bez QGIS, sprawdzany poza aplikacja.
...
QVariantMap NarzedziaProjektu::eksportujInwentaryzacjeDxf( QgsProject *projekt ) const
{
  QVariantMap wynik;
  QgsProject *p = projekt ? projekt : QgsProject::instance();
  if ( !p || !p->layerTreeRoot() )
  ...
  auto zapisz = [&]( const QString &sciezka, const QByteArray &bazowy ) -> bool {
    const DxfInwentaryzacja::Wynik w = DxfInwentaryzacja::dopisz( bazowy, lista, ust, bazowy.contains( "ANSI_1250" ) ? &doCp1250 : nullptr );
    uwagi << w.uwagi;
    if ( w.dxf.isEmpty() )
      return false;
    QFile plik( sciezka );
...
QVariantMap NarzedziaProjektu::stylujInwentaryzacje( QgsProject *projekt ) const
{
  QVariantMap wynik;
  QgsProject *p = projekt ? projekt : QgsProject::instance();
  if ( !p || !p->layerTreeRoot() )
